...
| `usage-status` | Color-coded `aboveEditor` widget row showing remaining usage + reset countdowns for every provider/account/meter `authStorage.fetchUsageReports` returns (data-driven, like `/usage`), not a fixed provider list. `selectMeters` groups a report's limits by `scope.tier`, so a model-specific quota (Codex Spark, Claude's Opus-only weekly) is its own entry; never key display off `metadata.planType`. |
