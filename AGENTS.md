...
| `usage-status` | Color-coded `aboveEditor` widget row showing remaining usage + reset countdowns for every provider/account/meter `authStorage.usage.reports` returns (data-driven, like `/usage`), not a fixed provider list. `selectMeters` groups a report's limits by `scope.tier`, so each meter (base, Codex Spark, Claude's Opus-only weekly) renders as its own entry. |
