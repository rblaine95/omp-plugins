...
| `usage-status` | Color-coded row above the editor showing remaining usage with reset countdowns for every provider, account, and meter `/usage` reports (Claude, Codex, Gemini, Grok, OpenCode, Cursor, …), so you don't have to run `/usage`. Base and model-specific meters read separately, so `Codex Spark` and `Claude Fable` (the Opus-only weekly) each get their own entry instead of overwriting the account-wide numbers. |
